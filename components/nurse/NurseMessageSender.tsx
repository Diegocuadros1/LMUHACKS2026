...
export function NurseMessageSender({ sessionId, patientName }: NurseMessageSenderProps) {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white shadow-sm p-3">
      ...
      {feedback && (
        <p className={`mb-2 rounded-xl px-3 py-1.5 text-xs font-medium ${
          feedback.ok
      ...
      <div className="flex gap-2">
        <input
          ...
          maxLength={500}
          disabled={loading}
          className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-200 disabled:opacity-50"
        />
        <button
          onClick={handleSend}
          disabled={!message.trim() || loading}
          className="rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 transition disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-emerald-400"
        >
          {loading ? '…' : 'Send'}
        </button>
      </div>
